...
import 'package:pay_unit_sdk/src/logger_utils.dart';
...
class ApiService {
  ...
  initiatePayment({
    ...
    try {
      //initiation pusher
      await initPusher(transactionId, actionAfterProccess, context);
      // await initializeNotifications();
      debugPrint("is sandbox : $sandbox");
      var data = jsonEncode({
      ...
      debugPrint("InitiatePayment catchError : $e");
      constant.makeToast("Something went wrong , please try again or later", context, Colors.red);
    } finally {
      closeDialog(context);
    }
  }
  ...
  getPaymentStatus({
    ...
    try {
      ...
      Timer timer = Timer.periodic(time, (Timer timer) async {
        var jsonResponse;
        var response = await getDigestAuth(merchandUserName, merchandPassword).get(
    ...
    try {
      ...
      if (provider_short_tag == 'mtnmomo' || provider_short_tag == 'orange') {
        ...
        response = await getDigestAuth(merchandUserName, merchandPassword).post(
            ...
            body: data,
            headers: getGlobalHeader(X_API_KEY));
      } else if (provider_short_tag == 'stripe' || provider_short_tag == 'paypal') {
        progressHub("Please wait", context);
        response = await getDigestAuth(merchandUserName, merchandPassword)
            .post("${sandbox == 'sandbox' ? constant.baseUrlSandBox : constant.baseUrl}/gateway/makepayment",
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (provider_short_tag == 'stripe') {
          var obj = {"secret_key": jsonResponse["secret_key"], "publishable_key": jsonResponse["publishable_key"]};
          return obj;
        }
  ...
  Future<void> initPusher(channelName, actionAfterProccess, BuildContext context) async {
    print("Transaction id pusher : $channelName");
    // print("Transaction actionAfterProcess : $actionAfterProcess");
    try {
      await pusher.init(apiKey: '8e6ec079f9d817bd73eb', cluster: 'ap4');
    } catch (e) {
      (e);
    }
    //connect to pusher
    ...
    void onConnectionStateChange(dynamic currentState, dynamic previousState) async {
      logInfo("Pusher connection state : $currentState");
    }

    void onError(String message, int? code, dynamic e) {
      logError("onError: $message code: $code exception: $e");
    }

// MethodChannel channel = MethodChannel('flutter.io/pusher');
    //connect to channel
    ...
    channel.bind('payment-event', (last) {
      ...
      if (jsonDecode(last.data)['transaction_status'] == "SUCCESS") {
        constant.makeToast(
            ...
            context,
            jsonDecode(last.data)['transaction_status'] == "SUCCESS" ? Colors.green : Colors.red);
        actionAfterProccess(jsonDecode(last.data)['transaction_id'], jsonDecode(last.data)['transaction_status']);
        closeDialog(context);
      } else {
  ...
  getGlobalHeader(String xApiKey) {
    return {
  ...
  Future<String> createCheckout(
    ...
    print(" transaction_id in checkout: $transactionId");
    final body = {
  ...
  Future<String> stripeCallBackTransactionIdAndStatus(
    ...
    var data = jsonEncode({"transaction_id": transactionId, "transaction_status": transactionStatus});
